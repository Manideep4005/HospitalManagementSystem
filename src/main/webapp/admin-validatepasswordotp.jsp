...
<body onload="document.querySelector('.f').reset();">
	...
	<div class="container">
		<form action="AdminForgotPasswordValidateOtp" method="POST"
			onsubmit="return check()" class="f">

			<p id="heading">Reset Password</p>

			<c:if test="${adminForgotOtpError != null}">
				<p id="message">${adminForgotOtpError }</p>
			</c:if>

			<c:if test="${adminOtpSentMail != null}">
				<p id="mailSent">${adminOtpSentMail }</p>
			</c:if>

			<input type="text" inputmode="numeric" id="country_code"
				name="adminOtp" pattern="[0-9]{6}"
				title="Enter six Digit Admin Id." maxlength="6" required
